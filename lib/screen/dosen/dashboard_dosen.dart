...
class DashboardDosen extends StatefulWidget {
  const DashboardDosen({Key? key}) : super(key: key);
...
class _DashboardDosenState extends State<DashboardDosen> {
  ...
  @override
    ...
    void loadDataDosen() async{
      String? username = await storage.read(key: Constanta().keyUsernameDosen);
    dosen.username = username!;
    var requestBody = jsonEncode(dosen);
    ModulDaringAPIService().getDosenDataByUsername(requestBody).then((result) {
      setState(() {
        isLoading = true;
      });
      if (result.success) {
        dosen = Dosen.fromJson(result.content);
        // loadDataMahasiswa();
      } else {
      ...
      child: ListView(
        children: [
          UserAccountsDrawerHeader(
            accountEmail: Text("Dosen1@gmail.com"),
            accountName: Text("Dosen1"),
          ...
          ListTile(
            ...
            title: Text("Logout"),
            onTap: () async{
              await storage.delete(key: Constanta().keyUsernameDosen);
              Navigator.push(context,
...
class _ChangePassState extends State<ChangePass> {
  ...
  void changePassword() async {
    ...
    ModulDaringAPIService()
        .getCPDosenDataByUsername(requestBody)
        .then((result) {
      if (result.success) {
        CoolAlert.show(
          context: context,
  ...
  void loadData() async {
    String? username = await storage.read(key: Constanta().keyUsernameDosen);
    userDosen.username = username!;
    var requestBody = jsonEncode(userDosen);
    print(requestBody);
    ModulDaringAPIService().getDosenDataByUsername(requestBody).then((result) {
      if (result.success) {
        userDosen = Dosen.fromJson(result.content);
      } else {
        print(result.message);
      }
      setState(() {
